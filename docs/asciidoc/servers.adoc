...
Certificates can be provided from any source using an `InputStream`:
...
----
{
  ...
  ssl.setCertificate(() -> new FileInputStream("path/to/certificate.p12"))
}
----
...
This allow to load certificates from disk, database or even memory.
...
{
  SslOptions ssl = SslOptions.pkcs12("path/to/server.p12", "password")
    .setTrustCertificatePath("path/to/trustCert") <1>
